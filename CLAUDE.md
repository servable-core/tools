...
**`preserveMostRestricted.js`'s `dedupeProtectedFields` used to throw `Cannot read properties of undefined (reading 'protectedFields')`** whenever a class's `classLevelPermissions` had never been set before (its first contribution from a protocol attaching to it) — it read `items[0].protectedFields`/`items[1].protectedFields` unconditionally, while `dedupePermissions` right next to it in the same file already guards the identical case. `protocolInClassClassSchema.js` catches this per-class (`console.error` + continue), so it never actually failed a build — the affected class just silently kept whatever `classLevelPermissions` it already had (often `undefined`) instead of the correctly-merged one. Found via a real boot of `backend/main`: 11 separate protocol/class pairs hit this in one build (mostly `disposableorphansable`/`disposablechildrenable` attaching to classes for the first time). Fixed to fall back to `{}` like its sibling does. Verified: same repro no longer throws, and a full build against backend/main's real tree goes from 11 occurrences to zero with an unchanged classes/hash result.
